...
########## TPLINK KASA KL130 #########
@command(["licht an", "lights on", "turn on the lights", "licht an", "lumos" ])
def lights_lights_on():

    #say_affirmative()
    bulb_turn_on("schlafzimmer")

@command(["off the lights", "turn off the lights", "licht aus" ])
def lights_lights_off():
    #say _affirmative()
    bulb_turn_off("schlafzimmer")
# Dimming 
@command(["dim up"])
def lights_up_10():
    bulb_dim_up_10("schlafzimmer")

@command(["dim down"])
def lights_down_10():
    bulb_dim_down_10("schlafzimmer")
